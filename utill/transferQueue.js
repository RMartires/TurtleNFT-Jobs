...
async function updateFulfillment(admin, orderId, data) {
    ...
    let locationId = null;
    if (fullData.body.fulfillments.legth > 0) {
        let fulfillmentId = fullData.body.fulfillments[0].id;
        locationId = fullData.body.fulfillments[0].location_id;
        await client.post({
        ...
        const locationsData = await client.get({
            path: 'locations',
        });
        locationId = locationsData.body.locations[0].id;
    }
